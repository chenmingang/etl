 ...
 * always has the same form and length within one version of MySQL. Each mysql.dbsync.event
 ...
 * <td>server_id</td>
 * <td>4 byte unsigned integer</td>
 * <td>Server ID of the server that created the mysql.dbsync.event.</td>
 * </tr>
 * 
 * <tr>
 * <td>total_size</td>
 * <td>4 byte unsigned integer</td>
 * <td>The total size of this mysql.dbsync.event, in bytes. In other words, this is the sum
 ...
 * <td>4 byte unsigned integer</td>
 * <td>The position of the next mysql.dbsync.event in the master binary log, in bytes from
 * the beginning of the file. In a binlog that is not a relay log, this is just
 * the position of the next mysql.dbsync.event, in bytes from the beginning of the file. In a
 * relay log, this is the position of the next mysql.dbsync.event in the master's binlog.</td>
 * </tr>
 * 
 * <tr>
 * <td>flags</td>
 ...
 * </table>
 * 
 * Summing up the numbers above, we see that the total size of the tracker.common header
 * is 19 bytes.
 * 
...
public final class LogHeader
{
    ...
    /**
     * The offset in the log where this mysql.dbsync.event originally appeared (it is
     * preserved in relay logs, making SHOW SLAVE STATUS able to print
     * coordinates of the mysql.dbsync.event in the master's binlog). Note: when a
     ...
     * is set at the mysql.dbsync.event's creation time, except for Query and Load (et al.)
     * events where this is set at the query's execution time, which guarantees
     * good replication (otherwise, we could have a query and its mysql.dbsync.event with
     ...
     * of the last seen FD mysql.dbsync.event.
     */
    protected int       checksumAlg;
    /**
       Placeholder for mysql.dbsync.event checksum while writing to binlog.
    */
            ...
            FormatDescriptionLogEvent descriptionEvent)
    {
        ...
        /*
          ...
          log_pos is the beginning of the mysql.dbsync.event: we transform it into the end
          of the mysql.dbsync.event, which is more useful.
        ...
        */
                ...
                && type < LogEvent.FORMAT_DESCRIPTION_EVENT && logPos != 0)
        {
            /*
              ...
              mysql.dbsync.event len's is nonsense. For example, a fake Rotate mysql.dbsync.event should
                ...
                || (type == LogEvent.ROTATE_EVENT))
        {
            ...
            */
            /*
              ...
              there will be an mysql.dbsync.event UID (but Format_description and Rotate
        ...
        If mysql.dbsync.event is FD the descriptor is in it.
        Notice, FD of the binlog can be only in one instance and therefore
        Show-Binlog-Events executing master side thread needs just to know
        the only FD's (A) value -  whereas RL can contain more.
        In the RL case, the alg is kept in FD_e (@description_event) which is reset 
        to the newer read-out mysql.dbsync.event after its execution with possibly new alg descriptor.
    ...
    /**
     * The position of the next mysql.dbsync.event in the master binary log, in bytes from
     * the beginning of the file. In a binlog that is not a relay log, this is
     * just the position of the next mysql.dbsync.event, in bytes from the beginning of the
     * file. In a relay log, this is the position of the next mysql.dbsync.event in the
     * master's binlog.
     */
    ...
    /**
     * The total size of this mysql.dbsync.event, in bytes. In other words, this is the sum
    ...
    /**
     * Server ID of the server that created the mysql.dbsync.event.
     */
