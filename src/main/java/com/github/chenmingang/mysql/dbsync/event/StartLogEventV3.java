 ...
 * Start_log_event of binlog format 4 (MySQL 5.0), that is, the mysql.dbsync.event that
 * describes the other events' Common-Header/Post-Header lengths. This mysql.dbsync.event is
 * sent by MySQL 5.0 whenever it starts sending a new binlog if the requested
 * position is >4 (otherwise if ==4 the mysql.dbsync.event will be sent naturally).
 * 
...
public class StartLogEventV3 extends LogEvent
{
    ...
    /* start mysql.dbsync.event post-header (for v3 and v4) */
