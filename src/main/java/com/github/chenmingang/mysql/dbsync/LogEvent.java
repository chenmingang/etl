...
/**
 * Binary log mysql.dbsync.event definitions. This includes generic code tracker.common to all types
 * of log events, as well as specific code for each type of log mysql.dbsync.event.
 * 
...
public abstract class LogEvent
{
     ...
     * start, other) to be sure an mysql.dbsync.event is not executed more than once in a
     ...
     * mysql.dbsync.event should be skipped or not. Example of ID: we already have the server id
     * (4 bytes), plus:
     * timestamp_when_the_master_started (4 bytes), a counter (a sequence number
     * which increments every time we write an mysql.dbsync.event to the binlog) (3 bytes).
    ...
    /**
     * These mysql.dbsync.event numbers were used for 5.1.0 to 5.1.15 and are therefore
     * obsolete.
     */
    ...
    /**
     * These mysql.dbsync.event numbers are used from 5.1.16 and forward
     */
    ...
    /**
     * Heartbeat mysql.dbsync.event to be send by master at its idle time to ensure master's online status to slave
     */
    ...
    /* New Maria mysql.dbsync.event numbers start from here */
    public static final int    ANNOTATE_ROWS_EVENT      = 160;
    /*
    Binlog checkpoint mysql.dbsync.event. Used for XA crash recovery on the master, not used
    in replication.
    A binlog checkpoint mysql.dbsync.event specifies a binlog file such that XA crash
    ...
    /*
    Gtid mysql.dbsync.event. For global transaction ID, used to start a new mysql.dbsync.event group,
    instead of the old BEGIN query mysql.dbsync.event, and also to mark stand-alone
    events.
     */
    public static final int    GTID_EVENT               = 162;
    /*
    Gtid list mysql.dbsync.event. Logged at the start of every binlog, to record the
    ...
    */
    ...
    /**
        For an mysql.dbsync.event, 'e', carrying a type code, that a slave,
        ...
        found an unknown mysql.dbsync.event in the relay log.
     */
    ...
    /**
     * Return mysql.dbsync.event header.
     */
