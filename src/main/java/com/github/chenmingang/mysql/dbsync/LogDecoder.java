...
import com.github.chenmingang.mysql.dbsync.event.mariadb.BinlogCheckPointLogEvent;
import com.github.chenmingang.mysql.dbsync.event.mariadb.MariaGtidListLogEvent;
import com.github.chenmingang.mysql.dbsync.event.mariadb.MariaGtidLogEvent;
 ...
 * do
 * {
 *     mysql.dbsync.event = decoder.decode(buffer, context);
 * 
...
public final class LogDecoder
{
    ...
    /**
     * Decoding an mysql.dbsync.event from binary-log buffer.
     *
     * @return <code>UknownLogEvent</code> if mysql.dbsync.event type is unknown or skipped,
     *         <code>null</code> if buffer is not including a full mysql.dbsync.event.
     */
        ...
        if (limit >= FormatDescriptionLogEvent.LOG_EVENT_HEADER_LEN)
        {
            ...
            if (limit >= len)
            {
                ...
                if (handleSet.get(header.getType()))
                {
                    ...
                    try
                    {
                        /* Decoding binary-log to mysql.dbsync.event */
    ...
    /**
     * Deserialize an mysql.dbsync.event from buffer.
     *
     * @return <code>UknownLogEvent</code> if mysql.dbsync.event type is unknown or skipped.
     */
                                  ...
                                  LogContext context) throws IOException
    {
        ...
        case LogEvent.SLAVE_EVENT: /* can never happen (unused mysql.dbsync.event) */
            {
                if (logger.isWarnEnabled())
        ...
        default:
            ...
            if((buffer.getUint16(LogEvent.FLAGS_OFFSET) & LogEvent.LOG_EVENT_IGNORABLE_F) > 0){
                ...
                if (logger.isWarnEnabled())
                    logger.warn("Skipping unrecognized binlog mysql.dbsync.event "
        ...
        /* updating position in context */
        logPosition.position = header.getLogPos();
        /* Unknown or unsupported log mysql.dbsync.event */
