package com.github.chenmingang.mysql.parser;

import com.github.chenmingang.core.AviaterRegexFilter;
import com.github.chenmingang.mysql.dbsync.LogEvent;
import com.github.chenmingang.mysql.dbsync.event.*;
import com.github.chenmingang.mysql.dbsync.event.mariadb.AnnotateRowsEvent;
...
public class LogEventConvert {
    ...
    private CanalEntry.Entry parseQueryEvent(QueryLogEvent event) {
        ...
        if (StringUtils.endsWithIgnoreCase(queryString, BEGIN)) {
            CanalEntry.TransactionBegin transactionBegin = createTransactionBegin(event.getSessionId());
            CanalEntry.Header header = createHeader(binlogFileName, event.getHeader(), "", "", null);
            ...
            CanalEntry.TransactionEnd transactionEnd = createTransactionEnd(0L); // MyISAM可能不会有xid事件
            CanalEntry.Header header = createHeader(binlogFileName, event.getHeader(), "", "", null);
            ...
            if (result.getType() == CanalEntry.EventType.ALTER || result.getType() == CanalEntry.EventType.ERASE
                    || result.getType() == CanalEntry.EventType.CREATE || result.getType() == CanalEntry.EventType.TRUNCATE
                    || result.getType() == CanalEntry.EventType.RENAME || result.getType() == CanalEntry.EventType.CINDEX
                    || result.getType() == CanalEntry.EventType.DINDEX) { // 针对DDL类型
                ...
                if (StringUtils.isEmpty(tableName)
                        || (result.getType() == CanalEntry.EventType.RENAME && StringUtils.isEmpty(result.getOriTableName()))) {
                    ...
                    if (nameFilter != null && !nameFilter.filter(schemaName + "." + tableName)) {
                        if (result.getType() == CanalEntry.EventType.RENAME) {
            ...
            } else if (result.getType() == CanalEntry.EventType.INSERT || result.getType() == CanalEntry.EventType.UPDATE
                    || result.getType() == CanalEntry.EventType.DELETE) {
            ...
            // 更新下table meta cache
            if (tableMetaCache != null
                    && (result.getType() == CanalEntry.EventType.ALTER || result.getType() == CanalEntry.EventType.ERASE || result.getType() == CanalEntry.EventType.RENAME)) {
            ...
            CanalEntry.Header header = createHeader(binlogFileName, event.getHeader(), schemaName, tableName, type);
            ...
            return createEntry(header, CanalEntry.EntryType.ROWDATA, rowChangeBuider.build().toByteString());
        }
    }
    ...
    private CanalEntry.Entry parseXidEvent(XidLogEvent event) {
        CanalEntry.TransactionEnd transactionEnd = createTransactionEnd(event.getXid());
        CanalEntry.Header header = createHeader(binlogFileName, event.getHeader(), "", "", null);
        return createEntry(header, CanalEntry.EntryType.TRANSACTIONEND, transactionEnd.toByteString());
    }
    ...
    private CanalEntry.Entry parseRowsEvent(RowsLogEvent event) {
        try {
            ...
            if (table == null) {
                // tableId对应的记录不存在
                throw new CanalParseException("not found tableId:" + event.getTableId());
            }
            ...
            CanalEntry.Header header = createHeader(binlogFileName,
                    event.getHeader(),
                    table.getDbName(),
                    table.getTableName(),
                    eventType);
    ...
    private void parseOneRow(CanalEntry.RowData.Builder rowDataBuilder, RowsLogEvent event, RowsLogBuffer buffer, BitSet cols,
    ...
    private CanalEntry.Entry buildQueryEntry(String queryString, LogHeader logHeader) {
        CanalEntry.Header header = createHeader(binlogFileName, logHeader, "", "", CanalEntry.EventType.QUERY);
        CanalEntry.RowChange.Builder rowChangeBuider = CanalEntry.RowChange.newBuilder();
        rowChangeBuider.setSql(queryString);
    ...
    private CanalEntry.Header createHeader(String binlogFile, LogHeader logHeader, String schemaName, String tableName,
    ...
    private boolean isUpdate(List<CanalEntry.Column> bfColumns, String newValue, int index) {
        ...
        CanalEntry.Column column = bfColumns.get(index);
